...
const (
	...
	OtherPlaceLoggedIn = 50050
	// OtherIPLoggedIn 异常IP登录
	OtherIPLoggedIn = 50051
	// OtherClientsLoggedIn 用户已经通过其他端登录
	OtherClientsLoggedIn = 50052
	// SessionTerminated 会话中断
	SessionTerminated = 50053
	// AccessTokenExpired token过期
	AccessTokenExpired = 50054
	// RefreshTokenExpired token刷新过期
	RefreshTokenExpired = 50055
	// AccessTokenIllegal 访问token不合法
	AccessTokenIllegal = 50056
	// RefreshTokenIllegal 刷新token不合法
	RefreshTokenIllegal = 50057
	// VerifyCodeRequired 需要验证码
	VerifyCodeRequired = 50058
	// PasswordExired 用户密码过期
	PasswordExired = 50059
...
func codeReason(c int) string {

	v, ok := reasonMap[c]

	if !ok {
		return reasonMap[UnKnownException]
	}
